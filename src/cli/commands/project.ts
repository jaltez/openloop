...
export function registerProjectCommands(cli: Argv): void {
  ...
  cli.command("project <command>", "Manage linked projects", (projectCli: Argv) =>
    projectCli
      .command(
      ...
      .command(
        ...
        (command: Argv) => command.option("format", { type: "string", choices: ["table", "json"] as const, describe: "Output format" }),

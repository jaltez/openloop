...
export function registerTaskCommands(cli: Argv): void {
  cli.command("task <command>", "Manage project tasks", (taskCli: Argv) =>
    taskCli
      .command(
        "add",
        ...
        async (args: AddTaskArgs) => {
          ...
          const rawScope = args.scope && args.scope.length > 0 ? [...new Set(args.scope.map((value) => String(value)))] : [];
          ...
          const task: ProjectTask = {
            ...
            source: ref ? { type: "issue", ref } : { type: "human", ref: "openloop task add" },
            specId: null,
            branch: null,
            owner: "openloop",
            ...
            dependsOn: args.dependsOn && args.dependsOn.length > 0 ? [...new Set(args.dependsOn.map((value) => String(value)))] : undefined,
      ...
      .command(
        ...
        (command: Argv) => command.option("project", { type: "string", alias: "p", demandOption: true }),
...
export function registerApprovalCommands(cli: Argv): void {
  cli.command("approval <command>", "Inspect human-approval packets for pending promotions", (approvalCli: Argv) =>
    approvalCli
      .command(
        "list",

...
export function registerDashboardCommands(cli: Argv): void {
  cli.command("dashboard <command>", "Web dashboard for monitoring", (dashCli: Argv) =>
    dashCli
      .command(
        "start",
        "Start the dashboard web server",
        (cmd: Argv) => cmd.option("port", { type: "number", describe: "Port to listen on (default: from config or 7399)" }),
        async (args) => {
      ...
      .command(
        ...
        (cmd: Argv) => cmd.option("port", { type: "number", describe: "Port to listen on" }),
        async (args) => {
          const config = await loadGlobalConfig();
          config.dashboard = {
